...
%% cahnge this to make it  easier to use
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item The probibility that at any given momnet 21 or more subscribers will be comunicating is 0.00794 or 0.794\%.
    \item Tranmsiion causes packet delays and packet lost. 
    \item Packet switching networks can handle more host and allocate bandwith.
    \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item If all the bits in an IPv4 address were used we could have $2^{32}$ adresses. 
  ...
  \begin{enumerate}
    ...
    \item $2^{128}$ is best represented in interger power of ten with $10^{38}$
  ...
  \begin{enumerate}
    \item The address format is a hexidecimal MAC address. 
  ...
  \begin{enumerate}
    \item The Transport layer segement headers include the port number. 
  ...
  \begin{enumerate}
    \item The main service that DHCP provides clients with a vaild IP address, subnet mask and default gateway. 
	...
	The client knows that the IP address is intened for them because the \emph{offer message} includes the client's MAC address. 
    \end{enumerate}
...
%look in book
  \begin{enumerate}
    \item NATs can't be used if there are a large number of hosts that are not controlled by the same person
    \item Making applications that work with NATs are more complicated
    \item NATs do not work with devices that don't support IPv4.
    \item New security standards may reject the cahnges that NATs make to the headers of packets.
    \end{enumerate}
...
the spanning tre. These lables can be used to traces the tree back to
it’s start from th eend, by following each node down the tree.
    \end{enumerate}
...
%Check answers plz
  ...
  \begin{enumerate}
    \item The receiver knows that a error has occured in the messgae.
  ...
  \begin{enumerate}
    ...
    \item ''Plug-and-play'' is used in switchs
  ...
  \begin{enumerate}
    ...
    \item The sender will send both the Data D, and the CRC remander (R), so that this division operation can be done. 
