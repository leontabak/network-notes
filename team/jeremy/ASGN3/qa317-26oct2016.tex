...
%% change this to make it  easier to use
%% OG: \documentclass[twoside]{article}
...
\begin{enumerate}
  ...
  \begin{enumerate}
    \item The probability that at any given moment 21 or more subscribers will be communicating is 0.00794 or 0.794\%.
    \item Transmission causes packet delays and packet lost. 
  ...
  \begin{enumerate}
    \item There are 32 bits in a IPv4 address.
    \item If all the bits in an IPv4 address were used we could have $2^{32}$ addresses. 
    \item The dotted decimal address of \verb+www.eonsahead.com+ is 45.40.136.115.
    \item The hexadecimal address of \verb+www.eonsahead.com+ is 2D288873.
    \item The binary address of \verb+www.eonsahead.com+ is 00101101001010001000100001110011.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The current population of Earth is about 7.125 billion people.
    \item $2^{128}$ is best represented in integer power of ten with $10^{38}$
    \item Each person on earth could have about $4.7*10^{28}$ IPv6 addresses.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The CIDR address block 10.0.0.0/8 and 192.168.0.0/16 are used for local comunications within a private network.
    \item There are 16,777,216 addresses in the 10.0.0.0/8 block.
    \item There are 65,536 addresses in the 192.168.0.0/16 block.
    \item The IPv4 address for the lab computer I used is 10.101.6.33. 
    \end{enumerate}
  ...
  \begin{answer}
    Port number is a way to specify the process that a packet should be sent to within a machine. 
    \end{answer}
  ...
  \begin{enumerate}
    \item The address format is a hexadecimal MAC address. 
    \item The hardware/physical address of a computer. 
    \item Mac addresses need 6-bytes or 48-bits. 
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The Transport layer segment headers include the port number. 
    \item The Network layer datagrams headers include the IP addresses. 
    \item The Link layer Ethernet frames headers include MAC addresses. 
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The main service that DHCP provides clients with a valid IP address, subnet mask and default gateway. 
    \item DHCP is especially useful in networks where hosts come and go.
    \item DHCP clients use UDP port number 67 to find servers, and port 68 to find clients. 
    \item The \emph{offer message} is sent to the client so that the client knows which IP address it has been given. 
	The client knows that the IP address is intended for them because the \emph{offer message} includes the client's MAC address. 
    \end{enumerate}
  ...
  \begin{enumerate}
    \item NATs use ports in ways other than their intended use of addressing process.
    \item Routers are only supposed to process packets up to layer 3 
    \item NATs violate end-to-end arguments.
    \item NATs and IPv6 are different ways to solve the IP address shortage and many people suggest using IPv6 rather than NATs.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item DNS translates IP addresses and Host name. 
    \item ARP translates IP addresses and physical addresses (MAC addresses)
    \end{enumerate}
  ...
  \begin{enumerate}
    \item MTU is the limit on a packet in the network layer. 
    \item MSS is the limit on a packet in the transport layer.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The number represents the distance between the that node and the
node at the start of the spanning tree.
    \item The label denotes the node one "step" closer to node at the start of
the spanning tree. These labels can be used to traces the tree back to
it’s start from the end, by following each node down the tree.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item Inter-AS routing is used to connect networks together. 
    \item Scalability is more important for inter-AS routing.
    \item Performance is more important for intra-AS routing.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item The receiver knows that a error has occurred in the message.
    \item The receiver can fix the error by looking at the bit that is in both the row and column that have the wrong parity bit, and change that bit to correct the error.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item $E$ increases when $d_{prog}$ is decreased.  
    \item $E$ increases when $d_{tran}$ is increased.
    \end{enumerate}

    \end{answer}

  \item Both routers and switches store and forward packets.
    ...
    Network administrators need to know the advantages and disadvantages of
    each kind of device.
  ...
  \begin{enumerate}
    ...
    \item Susceptibility to broadcast storms (an unattractive characteristic) is a property
      of which kind of device: router or switch?
    \end{enumerate}
  ...
  \begin{enumerate}
    \item Hierarchical addressing is used in routers.
    \item ''Plug-and-play'' is used in switches
    \item A router is faster at processing switch.
    \item A router has  greater variety of topologies.
    \item A switch is susceptible to broadcast storms.
    \end{enumerate}
  ...
  \begin{enumerate}
    \item This method of long division replaces subtraction with exclusive OR, XOR.
    \item The sender will send both the Data D, and the CRC remainder (R), so that this division operation can be done. 
    \item CRC can detect burst errors that are less than the generator's length long.
    \end{enumerate}
