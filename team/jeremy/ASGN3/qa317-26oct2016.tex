...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item When a customer attempts to use more that 150 kbps on a channel, they will burst. When bursting a customer's packets will be eligible to be dropped by the rouer. 
    \item We can infer from the result that an advantage of packet switched communications is that the network will not be overburdened due to the number of concurent users.
    \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item Each person on earth could have $4*10^{28}$ IPv6 addresses.
    \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item The address idetifes an Apple computer. 
  ...
  \begin{enumerate}
    \item The main service that DHCP provides clients with a vaild IP address. 
    \item DHCP is especially useful to assign many IP address to computers.
    \item DHCP clients use UDP port number 67 to find servers. 
    \item The \emph{offer message} is sent to the client so that the client knows which IP address it has been given. The client knows that the IP address is intened for them because the \emph{offer message} includes the client's MAC address. 
    \end{enumerate}
  ...
  \begin{enumerate}
    \item DNS translates IP addresses and Web addresses. 
...
%Check this problem plz
  \begin{enumerate}
    \item MTU is the limit on a packet in the network layer. 
    \item MSS is the limit on a packet in the network layer.
    \end{enumerate}
