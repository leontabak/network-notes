...
\begin{enumerate}
  ...
  \begin{enumerate}
    ...
    \item 4.7758929e+28 (a lot). (or $5*10^{28}$ according to \href{http://itknowledgeexchange.techtarget.com/whatis/ipv6-addresses-how-many-is-that-in-numbers/}{Site}
    \end{enumerate}
  ...
  \begin{answer}
    A port is an endpoint of communication, specific port numbers generally identify specific services such as webservers, Telnet, DNS, etc.  It is supposed to address processes.
    \end{answer}
  ...
  \begin{enumerate}
    \item Dynamic Host Configuration Protocol provides an Internet Protocol host with it's IP address and other information such as the subnet mask and default gateway of it's local network.  Also may give address for DNS servers and DNS domain name.  
    \item Especially useful when connecting a computer to a network for the first time and you don't want to manually create an IP address.
    \item Network protocol used on IP networks.  It is part of the application layer.  It is implemented with two UDP port numbers; UDP port number 67 is the destination port of a server, and UDP port number 68 is used by the client.
  ...
  \begin{enumerate}
    \item DNS translates between Host Name and IP addresses. (or vice versa)
  ...
  \begin{enumerate}
    \item The number is the 'cost' to get to the node
    \item The label is the node from which the cost to get at the current node is from (ie. if it is E then the cost is from E to the current node)
    \end{enumerate}
  ...
  \begin{enumerate}
    \item Communicating between two ASs, ability to obtain reachability nformation from neighboring ASs and propagating the reachability information to all routers internal to the AS 
  ...
  \begin{enumerate}
    \item Routers
    \item Switch
    \item Switch (faster speed, in LAN) Router (faster decisions, faster in MAN/WAN)
    \item Switch (connects two or more nodes) (While Routers connect two or more networks)
    \item Switch
    \end{enumerate}
  ...
  \begin{enumerate}
    ...
    \item D value (piece of data) 1010101010, additional R bits so that it is exactly divisible by the G (generator)
    \item Recieve divides the D+R by G and if it is a non-zero answer there are errors.  Thus it can detect burst errors of fewer than r+1 bits, and even so a burst error with r+1 bits is detected with probability of $1 - 0.5^{r}$.  Also it can detect any odd number of bit errors.
    \end{enumerate}
